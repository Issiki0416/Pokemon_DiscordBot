...
const pokemonData = require("../json/pokemon_data.json");
class Pokemon {
    pokeArray = [];
    sideColor = 0xffffff;
    thumbnailUrl = "";
    thumnailUrlRandom = 0;
    choosePokemon(content) {
    ...
    thumnailUrRandom(pokemon_id) {
        this.thumnailUrlRandom = this.getRandomArbitrary(0, 4096);
        if (this.thumnailUrlRandom == 1000) {
            this.thumbnailUrl = `https://www.serebii.net/Shiny/SM/${pokemon_id < 10 ? "00" : pokemon_id < 100 ? "0" : ""}${pokemon_id}.png`;
        }
        else {
            this.thumbnailUrl = ` https://www.serebii.net/sunmoon/pokemon/${pokemon_id < 10 ? "00" : pokemon_id < 100 ? "0" : ""}${pokemon_id}.png`;
        }
        return this.thumbnailUrl;
    }
    getMegaEvolution(data, thumbnailUrl) {
        if (data.isMegaEvolution) {
            ...
            switch (data.name) {
                ...
                case "メテノ":
                    if (data.form == "コア") {
                        let random = this.getRandomArbitrary(0, 7);
                        if (random == 0) {
                            return thumbnailUrl.slice(0, -4) + "-b.png";
                        }
                        else if (random == 1) {
